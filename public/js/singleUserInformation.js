...
deleteMorakhasiBtn.addEventListener("click", async () => {
  try {
    const result = await axios.post(
      `/duty/admin/${codePersoneli.textContent}?action=deleteMorakhasi`
    );
...
stopWorking.addEventListener("click", async () => {
  if (duty.innerHTML == "on") {
    await axios.post(`/duty/?action=logout`, {
      codePersoneli: codePersoneli.textContent,
    });
  } else if (duty.innerHTML == "off") {
    await axios.post(`/duty/?action=login`, {
      codePersoneli: codePersoneli.textContent,
    });
  }
  location.reload();
...
deleteUser.addEventListener("click", async () => {
  if (confirm("کاربر مورد نظر حذف شود ؟")) {
    try {
      const result = await axios.post(
        `/duty/admin/${codePersoneli.textContent}?action=deleteUser`
      );
      if (result.data == true) location.href = "/duty/admin";
    } catch (error) {}
  }
});
